...
/**
 * Base class that represents a row from the 'modality' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class Modality implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ModalityTableMap';
    ...
    /**
     * Initializes internal state of Base\Modality object.
     */
    ...
    /**
     * Compares this with another <code>Modality</code> instance.  If
     ...
     * @return $this|\Modality The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ModalityTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[ModalityTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ModalityTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ModalityTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = ModalityTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Modality'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildModalityQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Modality::setDeleted()
     * @see Modality::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildModalityQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                ModalityTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[ModalityTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . ModalityTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(ModalityTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ModalityTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }

        $sql = sprintf(
            'INSERT INTO modality (%s) VALUES (%s)',
    ...
    {
        $pos = ModalityTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Modality'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Modality'][$this->hashCode()] = true;
        $keys = ModalityTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
        );
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Modality The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(ModalityTableMap::DATABASE_NAME);

        if ($this->isColumnModified(ModalityTableMap::COL_ID)) {
            $criteria->add(ModalityTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ModalityTableMap::COL_NAME)) {
            $criteria->add(ModalityTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildModalityQuery::create();
        $criteria->add(ModalityTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Modality (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Modality Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildModality is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildModality The current object (for fluent API support)
     */
     ...
     * Otherwise if this Modality is new, it will return
     * an empty collection; or if this Modality has previously
     * been saved, it will retrieve related Visits from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Modality.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(ModalityTableMap::DEFAULT_STRING_FORMAT);
    }
