...
/**
 * Base class that represents a row from the 'walkin_hour' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class WalkinHour implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\WalkinHourTableMap';
    ...
    /**
     * Initializes internal state of Base\WalkinHour object.
     */
    ...
    /**
     * Compares this with another <code>WalkinHour</code> instance.  If
     ...
     * @return $this|\WalkinHour The current object (for fluent API support)
     */
    public function setAdvisorId($v)
    {
        ...
        if ($this->advisor_id !== $v) {
            $this->advisor_id = $v;
            $this->modifiedColumns[WalkinHourTableMap::COL_ADVISOR_ID] = true;
        }
    ...
    public function setStartsAt($v)
    {
        ...
        if ($this->starts_at !== null || $dt !== null) {
            if ($this->starts_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->starts_at->format("Y-m-d H:i:s.u")) {
                $this->starts_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[WalkinHourTableMap::COL_STARTS_AT] = true;
            }
    ...
    public function setEndsAt($v)
    {
        ...
        if ($this->ends_at !== null || $dt !== null) {
            if ($this->ends_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->ends_at->format("Y-m-d H:i:s.u")) {
                $this->ends_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[WalkinHourTableMap::COL_ENDS_AT] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : WalkinHourTableMap::translateFieldName('AdvisorId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->advisor_id = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : WalkinHourTableMap::translateFieldName('StartsAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->starts_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : WalkinHourTableMap::translateFieldName('EndsAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 3; // 3 = WalkinHourTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\WalkinHour'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildWalkinHourQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see WalkinHour::setDeleted()
     * @see WalkinHour::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildWalkinHourQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                WalkinHourTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(WalkinHourTableMap::COL_ADVISOR_ID)) {
            $modifiedColumns[':p' . $index++]  = 'advisor_id';
        }
        if ($this->isColumnModified(WalkinHourTableMap::COL_STARTS_AT)) {
            $modifiedColumns[':p' . $index++]  = 'starts_at';
        }
        if ($this->isColumnModified(WalkinHourTableMap::COL_ENDS_AT)) {
            $modifiedColumns[':p' . $index++]  = 'ends_at';
        }

        $sql = sprintf(
            'INSERT INTO walkin_hour (%s) VALUES (%s)',
    ...
    {
        $pos = WalkinHourTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['WalkinHour'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['WalkinHour'][$this->hashCode()] = true;
        $keys = WalkinHourTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getAdvisorId(),
            $keys[1] => $this->getStartsAt(),
            $keys[2] => $this->getEndsAt(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setAdvisorId($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setStartsAt($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setEndsAt($arr[$keys[2]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\WalkinHour The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(WalkinHourTableMap::COL_ADVISOR_ID)) {
            $criteria->add(WalkinHourTableMap::COL_ADVISOR_ID, $this->advisor_id);
        }
        if ($this->isColumnModified(WalkinHourTableMap::COL_STARTS_AT)) {
            $criteria->add(WalkinHourTableMap::COL_STARTS_AT, $this->starts_at);
        }
        if ($this->isColumnModified(WalkinHourTableMap::COL_ENDS_AT)) {
            $criteria->add(WalkinHourTableMap::COL_ENDS_AT, $this->ends_at);
        }
    ...
    {
        $criteria = ChildWalkinHourQuery::create();
        $criteria->add(WalkinHourTableMap::COL_ADVISOR_ID, $this->advisor_id);
        $criteria->add(WalkinHourTableMap::COL_STARTS_AT, $this->starts_at);
        $criteria->add(WalkinHourTableMap::COL_ENDS_AT, $this->ends_at);
    ...
    {
        $validPk = null !== $this->getAdvisorId() &&
            null !== $this->getStartsAt() &&
            null !== $this->getEndsAt();

        $validPrimaryKeyFKs = 1;
        $primaryKeyFKs = [];

        //relation walkin_hour_fk_17b25f to table employee
        if ($this->aEmployee && $hash = spl_object_hash($this->aEmployee)) {
            $primaryKeyFKs[] = $hash;
        } else {
    ...
    {
        $pks = array();
        $pks[0] = $this->getAdvisorId();
        $pks[1] = $this->getStartsAt();
        $pks[2] = $this->getEndsAt();
    ...
    {
        $this->setAdvisorId($keys[0]);
        $this->setStartsAt($keys[1]);
        $this->setEndsAt($keys[2]);
    }
    ...
    {
        return (null === $this->getAdvisorId()) && (null === $this->getStartsAt()) && (null === $this->getEndsAt());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \WalkinHour (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \WalkinHour Clone of current object.
     * @throws PropelException
     */
    ...
    public function getEmployee(ConnectionInterface $con = null)
    {
        if ($this->aEmployee === null && (($this->advisor_id !== "" && $this->advisor_id !== null))) {
            ...
            /* The following can be used additionally to
                ...
                $this->aEmployee->addWalkinHours($this);
             */
        }
    ...
    {
        if (null !== $this->aEmployee) {
            $this->aEmployee->removeWalkinHour($this);
        }
    ...
    {
        return (string) $this->exportTo(WalkinHourTableMap::DEFAULT_STRING_FORMAT);
    }
