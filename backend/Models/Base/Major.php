...
/**
 * Base class that represents a row from the 'major' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class Major implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\MajorTableMap';
    ...
    /**
     * Initializes internal state of Base\Major object.
     */
    ...
    /**
     * Compares this with another <code>Major</code> instance.  If
     ...
     * @return $this|\Major The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[MajorTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[MajorTableMap::COL_NAME] = true;
        }
    ...
    /**
     * Sets the value of the [grad] column.
    ...
    public function setGrad($v)
    {
        ...
        if ($this->grad !== $v) {
            $this->grad = $v;
            $this->modifiedColumns[MajorTableMap::COL_GRAD] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : MajorTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MajorTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : MajorTableMap::translateFieldName('Grad', TableMap::TYPE_PHPNAME, $indexType)];
            $this->grad = (null !== $col) ? (boolean) $col : null;
            $this->resetModified();
            ...
            return $startcol + 3; // 3 = MajorTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Major'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildMajorQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildMajorQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                MajorTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MajorTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(MajorTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(MajorTableMap::COL_GRAD)) {
            $modifiedColumns[':p' . $index++]  = 'grad';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'name':
                        ...
                        break;
                    case 'grad':
                        $stmt->bindValue($identifier, (int) $this->grad, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = MajorTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Major'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Major'][$this->hashCode()] = true;
        $keys = MajorTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
            $keys[2] => $this->getGrad(),
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Major The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(MajorTableMap::DATABASE_NAME);

        if ($this->isColumnModified(MajorTableMap::COL_ID)) {
            $criteria->add(MajorTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(MajorTableMap::COL_NAME)) {
            $criteria->add(MajorTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(MajorTableMap::COL_GRAD)) {
            $criteria->add(MajorTableMap::COL_GRAD, $this->grad);
        }
    ...
    {
        $criteria = ChildMajorQuery::create();
        $criteria->add(MajorTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Major (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Major Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildMajor is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildMajor The current object (for fluent API support)
     */
     ...
     * Otherwise if this Major is new, it will return
     * an empty collection; or if this Major has previously
     * been saved, it will retrieve related Students from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Major.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildStudent}> List of ChildStudent objects
     */
    public function getStudentsJoinEmployee(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildStudentQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(MajorTableMap::DEFAULT_STRING_FORMAT);
    }
